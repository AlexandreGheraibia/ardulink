   ...
   if(currentPos<strlen(message)){
       if (millis() > (timer + patternChangeTime)) {
           ...
           if(mustMove){
             ...
             incremPendaPosition=45.0/tailleTotal;//30 degres d'intervalle de deplacement
             pendaMove();
          }  
       }
       sevseg.refreshDisplay(); // Must run repeatedly
          ...
          if(mustMove){
            incremPendaPosition=-incremPendaPosition;
          }
...
void pendaMove(){ 
  if(mustMove){ 
    if(!monServomoteur.attached()){
       ...
       position=position+incremPendaPosition;
